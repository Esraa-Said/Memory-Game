...
import { IPrepare } from "../models/prepareModel";
//#endregion
...
const changeProgress = () => {
  const progress =
    (prepare.cards.filter((card) => !card.clicked).length / numberOfCards) *
    100;
  ...
  cardsHtmlContent += `
    <span class = "col-sm-4 col-lg-3">
        ...
        <img src= "../assets/images/image_${
          item.index + 1
        }.png" alt = "Image"  style="height:120px;width:100%; display:block;">
        </div>
        </div>
        </div>
        </div>
